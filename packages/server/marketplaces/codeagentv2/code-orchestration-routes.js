 ...
 */
router.post('/generate', ensureInitialized, validateRequest(['userId', 'prompt']), async (req, res) => {
    try {
        const {
            userId,
            prompt,
 ...
 */
router.post('/sessions', ensureInitialized, validateRequest(['userId', 'language']), async (req, res) => {
    try {
        const { userId, language, framework = null, context = {}, description = null } = req.body
 ...
 */
router.get('/sessions/:sessionId/analytics', ensureInitialized, async (req, res) => {
    try {
 ...
 */
router.get('/sessions/:sessionId/tasks', ensureInitialized, async (req, res) => {
    try {
        const { sessionId } = req.params
 ...
 */
router.get('/sessions/:sessionId/followups', ensureInitialized, async (req, res) => {
    try {
        const { sessionId } = req.params
 ...
 */
router.post('/sessions/:sessionId/followups', ensureInitialized, validateRequest(['type']), async (req, res) => {
    try {
        const { sessionId } = req.params
        const { type, priority = 'medium', scheduledAt = null, context = {}, customMessage = null, taskId = null } = req.body
 ...
 */
router.get('/agents', ensureInitialized, async (req, res) => {
    try {
 ...
 */
router.post('/agents/execute', ensureInitialized, validateRequest(['sessionId', 'workflowId', 'prompt']), async (req, res) => {
    try {
        const { sessionId, workflowId, prompt, context = {}, agentOverrides = {} } = req.body
 ...
 */
router.get('/metrics', ensureInitialized, async (req, res) => {
    try {
 ...
 */
router.get('/document-stores', ensureInitialized, async (req, res) => {
    try {
        if (!integrationLayer.orchestrator) {
            return res.status(503).json({
                success: false,
 ...
 */
router.get('/document-stores/:id/loaders', ensureInitialized, async (req, res) => {
    try {
        const { id } = req.params
