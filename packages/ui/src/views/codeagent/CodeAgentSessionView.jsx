...
const CodeAgentSessionView = () => {
    ...
    const loadSessionData = async () => {
        try {
            setIsLoading(true)
            await Promise.all([loadSession(), loadSessionTasks(), loadSessionFollowUps(), loadCodeAnalysis()])
        } catch (error) {
    ...
    const renderSessionHeader = () => (
        <Box sx={{ mb: 3 }}>
            <Breadcrumbs sx={{ mb: 2 }}>
                <Link component='button' variant='body1' onClick={() => navigate('/codeagent')} sx={{ textDecoration: 'none' }}>
                    CodeAgent
                </Link>
                <Typography color='text.primary'>Session {sessionId}</Typography>
            </Breadcrumbs>
                    ...
                    <Stack spacing={2}>
                        <Box display='flex' alignItems='center' justifyContent='space-between'>
                            <Box display='flex' alignItems='center' gap={2}>
                                ...
                                <Typography variant='h5'>{session?.context?.description || `Session ${sessionId}`}</Typography>
                            </Box>
                            <Chip
                        ...
                        <Grid container spacing={2}>
                            <Grid item xs={12} sm={6} md={3}>
                                ...
                                <Typography variant='body1'>{session?.language || 'N/A'}</Typography>
                            </Grid>
                            ...
                            <Grid item xs={12} sm={6} md={3}>
                                ...
                                <Typography variant='body1'>{session?.framework || 'None'}</Typography>
                            </Grid>
                            ...
                            <Grid item xs={12} sm={6} md={3}>
                                ...
                                <Typography variant='body1'>{tasks.length} tasks</Typography>
                            </Grid>
                        </Grid>
                        ...
                        {session?.context?.prompt && (
                            <Box>
                                ...
                                <Paper sx={{ p: 2, bgcolor: theme.palette.grey[50] }}>
                                    <Typography variant='body2'>{session.context.prompt}</Typography>
                                </Paper>
                            </Box>
                        )}
                    </Stack>
        ...
        <Stack spacing={3}>
            {tasks.length === 0 ? (
                <Paper sx={{ p: 4, textAlign: 'center' }}>
                    ...
                    <Typography variant='body2' color='text.secondary'>
                        This session does not have any tasks yet.
                    </Typography>
                </Paper>
            ) : (
                                                    ...
                                                    <IconButton
                                                        ...
                                                        onClick={() =>
                                                            handleDownloadCode(
                                                                ...
                                                                `task_${index + 1}_${task.agent_type}.${
                                                                    session?.language === 'python' ? 'py' : 'js'
                                                                }`
                                                            )
                                                        }
                                                    >
    ...
    const renderFollowUpsTab = () => (
        <Stack spacing={3}>
            <Box display='flex' justifyContent='flex-end'>
                <Button variant='contained' startIcon={<IconSend />} onClick={() => setShowFollowUpDialog(true)}>
                    Request Follow-up
                </Button>
            </Box>
                    ...
                    <Card key={followUp.id}>
                        <CardContent>
                            <Stack spacing={2}>
                                <Box display='flex' alignItems='center' justifyContent='space-between'>
                                    <Box display='flex' alignItems='center' gap={2}>
                                        <Typography variant='h6'>{followUp.type.replace('_', ' ').toUpperCase()}</Typography>
                                        <Chip label={followUp.status} size='small' color={getStatusColor(followUp.status)} />
                                        <Chip label={followUp.priority} size='small' variant='outlined' />
                                    </Box>
                                ...
                                {followUp.result && (
                                    <Box>
                                        ...
                                        <Paper sx={{ p: 2, bgcolor: theme.palette.grey[50] }}>
                                            <Typography variant='body2'>{followUp.result}</Typography>
                                        </Paper>
                                    </Box>
                                )}
                            </Stack>
                            ...
                            <Stack spacing={2}>
                                <Box display='flex' alignItems='center' justifyContent='space-between'>
                                    ...
                                    <Chip
                                        ...
                                        color={
                                            analysis.overall_score >= 80 ? 'success' : analysis.overall_score >= 60 ? 'warning' : 'error'
                                        }
                                    />
                                </Box>
        ...
        return (
            <MainCard>
                <Box sx={{ textAlign: 'center', py: 4 }}>
                    ...
                    <Button variant='contained' startIcon={<IconArrowLeft />} onClick={() => navigate('/codeagent')}>
                        Back to CodeAgent
                    </Button>
                </Box>
            </MainCard>
    ...
    return (
        <>
            ...
            {/* Follow-up Dialog */}
            <Dialog open={showFollowUpDialog} onClose={() => setShowFollowUpDialog(false)} maxWidth='md' fullWidth>
                ...
                <DialogActions>
                    ...
                    <Button variant='contained' onClick={handleScheduleFollowUp} disabled={isProcessingFollowUp || !followUpMessage.trim()}>
