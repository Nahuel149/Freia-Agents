...
const formatToOpenAIAssistantTool = (tool: any): OpenAI.Beta.FunctionTool => {
    ...
    const addItemsToArrays = (schema: JSONSchema) => {
        ...
        if (schema.items && typeof schema.items === 'object') {
            addItemsToArrays(schema.items as JSONSchema)
        }
    }

    addItemsToArrays(parameters)

    console.log(`[SCHEMA DEBUG] Converted JSON schema for '${tool.name}':`, JSON.stringify(parameters, null, 2))
