...
import { utilAddChatMessage } from '../../utils/addChatMessage'
...
const removeAllChatMessages = async (
    ...
    try {
        ...
        if (chatId) {
            try {
                const { totalSize } = await removeFilesFromStorage(chatflowid, chatId)
                await updateStorageUsage('', '', totalSize, usageCacheManager)
            } catch (e) {
