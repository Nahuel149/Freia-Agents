...
class CodeAgentDefinitions {
    ...
    initializeAgentTypes() {
        ...
        this.agentTypes.set('documentation_agent', {
            ...
            capabilities: ['api_documentation', 'code_comments', 'readme_generation', 'technical_specifications', 'user_guides'],
            supportedLanguages: ['javascript', 'typescript', 'python', 'java', 'go', 'rust'],
            priority: 5,
            timeout: 20000,
    ...
    getAgentsByCapability(capability) {
        return Array.from(this.agentTypes.values()).filter((agent) => agent.capabilities.includes(capability))
    }
    ...
    getAgentsByLanguage(language) {
        return Array.from(this.agentTypes.values()).filter(
            (agent) => !agent.supportedLanguages || agent.supportedLanguages.includes(language)
        )
    }
    ...
    getAgentsByFramework(framework) {
        return Array.from(this.agentTypes.values()).filter(
            (agent) => !agent.supportedFrameworks || agent.supportedFrameworks.includes(framework)
        )
    }
