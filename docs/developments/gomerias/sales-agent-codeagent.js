...
class TireSalesAgent {
    ...
    createFollowUpPlan(clientId) {
        return {
            ...
            actions: ['Llamada de seguimiento', 'Envío de cotización detallada', 'Programar visita si es necesario']
        }
    }
    ...
    try {
        ...
        if (context.chatHistory && Array.isArray(context.chatHistory)) {
            context.chatHistory.forEach((msg) => {
                if (msg.content) {
                    agent.conversationHistory.addMessage(msg.type === 'user' ? 'user' : 'assistant', msg.content, { source: 'codeagent' })
                }
            })
        }
        ...
        if (!input || !input.trim()) {
            response =
                `🚗 ¡Bienvenido a Gomería "El Rutero Experto"! 🛞\n\n` +
        ...
        const errorMsg =
            `❌ Error en el agente de ventas: ${error.message}\n\n` +
