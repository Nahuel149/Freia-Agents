...
class TireSalesAgent {
        ...
        return [
            {
                ...
                response:
                    'Entiendo su preocupación por el precio. Nuestros productos ofrecen un 15% más de durabilidad que la competencia, lo que significa menor costo por kilómetro. ¿Le gustaría que le muestre el análisis de costo total de propiedad?',
                evidence: 'Datos de rendimiento de clientes similares'
            },
            {
                objection: 'Ya tengo proveedor',
                response:
                    'Perfecto, eso habla bien de su planificación. No pretendo que cambie inmediatamente. ¿Podríamos ser su proveedor de respaldo para emergencias? Así puede evaluar nuestro servicio sin comprometer su operación actual.',
                evidence: 'Casos de éxito como proveedor secundario'
            },
            {
                objection: 'Necesito pensarlo',
                response:
                    'Por supuesto, es una decisión importante. ¿Hay algún aspecto específico que le genera dudas? Me gustaría asegurarme de que tenga toda la información necesaria para tomar la mejor decisión.',
                evidence: 'Información adicional específica'
            },
            {
                objection: 'No tengo presupuesto ahora',
                response:
                    'Comprendo perfectamente. ¿Cuándo sería un mejor momento para revisar esto? Mientras tanto, puedo mantenerle reservado el stock y avisarle si hay alguna promoción especial.',
    ...
    searchProducts(query, limit = 10) {
        ...
        const results = this.businessData.products
            .map((product) => {
    ...
    semanticSearch(query, dataTypes = ['products', 'clients', 'sales']) {
        ...
        if (dataTypes.includes('clients') && this.businessData.clients) {
            results.clients = this.businessData.clients
                .filter((client) => {
                    const clientText = `${client.business_name} ${client.contact_person || ''} ${client.email || ''} ${
                        client.phone || ''
                    }`.toLowerCase()
        ...
        if (dataTypes.includes('sales') && this.businessData.sales) {
            results.sales = this.businessData.sales
                .filter((sale) => {
    ...
    getRecentSales(limit = 5) {
        ...
        return this.businessData.sales.sort((a, b) => new Date(b.sale_date) - new Date(a.sale_date)).slice(0, limit)
    }
...
if (require.main === module) {
    ...
    console.log('\n🎯 Example Consultation:')
    agent.provideSalesConsultation('Necesito cubiertas para Toyota Hilux', 'BIZ-001').then((consultation) => {
...
class WebSearchService {
    ...
    async searchGoogle(query, maxResults, language) {
        ...
        return {
            results:
                data.items?.slice(0, maxResults).map((result) => ({
                    title: result.title,
                    url: result.link,
    ...
    async searchTireInfo(query, vehicleModel = '', language = 'es') {
        ...
        const searchQuery = vehicleModel ? `${query} ${vehicleModel} ${tireTerm}` : `${query} ${tireTerm}`
...
async function llmComplete({ system, user, messages = [], mode = 'default', temperature = null }) {
    ...
    try {
        ...
        if (system && mode !== 'think') {
            ...
            console.log(
                '🔍 DEBUG - System prompt language check:',
                system.includes('You are') ? 'English' : system.includes('Eres') ? 'Spanish' : 'Unknown'
            )
        }
...
function determineProcessingMode(userInput, explicitMode = 'auto') {
    ...
    if (
        input.startsWith('/deepersearch ') ||
        input.startsWith('/dds ') ||
        input.startsWith('\\deepersearch ') ||
        input.startsWith('\\dds ')
    ) {
        return 'deepersearch'
    ...
    if (
        input.includes('búsqueda exhaustiva') ||
        input.includes('análisis completo') ||
        deeperSearchKeywords.some((keyword) => input.includes(keyword))
    ) {
        return 'deepersearch'
    }

    if (
        input.includes('búsqueda profunda') ||
        input.includes('información detallada') ||
        deepSearchKeywords.some((keyword) => input.includes(keyword))
    ) {
        return 'deepsearch'
    }
...
async function processDeepSearchRequest(userInput, messages, systemPrompt, context) {
    ...
    if (contextualInfo.relevantProducts.length > 0) {
        ...
        contextualInfo.relevantProducts.forEach((product) => {
            enhancedPrompt += `- ${product.name} (${product.brand}) - $${product.price} - ${localizedMessages.stock || 'Stock'}: ${
                product.stock
            }\n`
        })
        enhancedPrompt += `\n`
    ...
    enhancedPrompt += `- ${localizedMessages.mainCategories || 'Categorías principales'}: ${contextualInfo.businessInsights.topCategories
        .map((c) => c.category)
        .join(', ')}\n\n`
    ...
    enhancedPrompt += `${
        localizedMessages.useAllInfo || 'Utiliza toda esta información para proporcionar una respuesta completa y detallada'
    }.`
...
function getLocalizedMessages(language) {
    const messages = {
        es: {
            ...
            welcomeMessage:
                '¡Hola! Soy El Rutero Experto, tu asistente de ventas especializado en neumáticos. ¿En qué puedo ayudarte hoy? Puedo ayudarte con:\n\n• Consultas sobre productos y precios\n• Análisis de clientes y historial de compras\n• Recomendaciones personalizadas\n• Estrategias de venta\n• Manejo de objeciones\n\n¿Qué necesitas?',
            ...
            purchaseIntent:
                'El cliente muestra intención de compra. Enfócate en cerrar la venta con propuestas concretas y manejo de objeciones.',
        ...
        en: {
            ...
            welcomeMessage:
                'Hello! I am El Rutero Experto, your specialized tire sales assistant. How can I help you today? I can assist you with:\n\n• Product and pricing inquiries\n• Client analysis and purchase history\n• Personalized recommendations\n• Sales strategies\n• Objection handling\n\nWhat do you need?',
        ...
        pt: {
            ...
            welcomeMessage:
                'Olá! Sou El Rutero Experto, seu assistente de vendas especializado em pneus. Como posso ajudá-lo hoje? Posso ajudá-lo com:\n\n• Consultas sobre produtos e preços\n• Análise de clientes e histórico de compras\n• Recomendações personalizadas\n• Estratégias de vendas\n• Tratamento de objeções\n\nO que você precisa?',
