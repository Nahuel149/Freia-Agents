...
class AgentOrchestrator {
    ...
    async callAgentflow(agentId, input) {
        ...
        const responses = {
            'agentAgentflow_0': {
                response: `Hola ${input.customerContext?.customer?.name || 'Cliente'}! Te ayudo con ${input.message}. Tengo varias opciones que te pueden interesar.`,
                confidence: 0.9,
    ...
    async evaluateAgentCondition(agentConfig, previousResult, context) {
        ...
        if (agentConfig.role === 'price_negotiation') {
            ...
            return context.message.toLowerCase().includes('precio') || 
                   context.message.toLowerCase().includes('costo');
        }
    ...
    async processFinalResult(executionResult, customerContext, flowState) {
        try {
            ...
            const postAgents = executionResult.results
                .map(r => r.agent)
                .filter(name => name.includes('followup'));
