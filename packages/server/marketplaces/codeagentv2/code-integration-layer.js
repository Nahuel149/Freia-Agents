...
class CodeAgentIntegrationLayer {
    ...
    async generateCodeWithOrchestration(request) {
        try {
            ...
            const useOrchestration = this.config.features.enableOrchestration && 
                                   this.shouldUseOrchestration(request);
    ...
    async generateCodeTraditional(request) {
        ...
        const taskResult = await this.pool.query(taskQuery, [
            sessionId, userId, prompt, language, framework, 'pending'
        ]);
        ...
        try {
            ...
            await this.pool.query(
                'UPDATE code_tasks SET status = $1, error_message = $2, updated_at = CURRENT_TIMESTAMP WHERE id = $3',
    ...
    async updateSessionActivity(sessionId) {
        try {
            await this.pool.query(
                'UPDATE code_sessions SET last_activity_at = CURRENT_TIMESTAMP WHERE id = $1',
                [sessionId]
            );
    ...
    shouldUseOrchestration(request) {
        ...
        const complexKeywords = [
            'architecture', 'system', 'database', 'api', 'microservice',
            'authentication', 'authorization', 'security', 'performance',
            'scalability', 'integration', 'deployment'
        ];
        ...
        try {
            ...
            return {
                ...
                statistics: {
                    ...
                    successRate: analytics.total_tasks > 0 ? 
                        (analytics.completed_tasks / analytics.total_tasks * 100).toFixed(2) : 0
                }
            };
    ...
    async cleanupInactiveSessions() {
        ...
        for (const [sessionId, session] of this.activeSessions.entries()) {
            if (now - session.lastActivity > timeoutMs) {
                ...
                // Update database
                await this.pool.query(
                    'UPDATE code_sessions SET status = $1 WHERE id = $2',
                    ['inactive', sessionId]
                );
