...
export class ModifyChatMessageExecutionIdType1757800000001 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        if (hasColumn) {
            await queryRunner.query(
                `ALTER TABLE "chat_message" ALTER COLUMN "executionId" TYPE uuid USING NULLIF("executionId", '')::uuid`
            )
        }
    }
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        if (hasColumn) {
            await queryRunner.query(
                `ALTER TABLE "chat_message" ALTER COLUMN "executionId" TYPE varchar USING "executionId"::varchar`
            )
        }
    }
}
