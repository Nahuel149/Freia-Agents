...
const applyInventoryUpdate = async (productId: string, updates: InventoryUpdates) => {
    ...
    const existingProduct = await appServer.AppDataSource.query('SELECT * FROM product_inventory WHERE "productId" = $1', [productId])
...
const getInventoryAlternatives = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (productId) {
            const referenceResult = await appServer.AppDataSource.query('SELECT * FROM product_inventory WHERE "productId" = $1', [
                productId
            ])
        ...
        const alternatives = candidates
            .map((candidate: any) => {
                ...
                if (
                    category &&
                    (candidate.name?.toLowerCase().includes(category.toLowerCase()) ||
                        candidate.productId?.toLowerCase().includes(category.toLowerCase()))
                ) {
                    similarityScore += 0.1
                }
...
const reserveInventory = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        const { productId, quantity, customerId, phoneNumber, reason, expiresAt, agentId, notes } = req.body
        ...
        const productRows = await client.query('SELECT * FROM product_inventory WHERE "productId" = $1 FOR UPDATE', [productId])
        ...
        await client.query('UPDATE product_inventory SET stock = $1, "updatedDate" = NOW() WHERE "productId" = $2', [
            updatedStock,
            productId
        ])
...
const notifyWhenInStock = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const { productId, customerId, clientId, phone_number, notificationType = 'stock_available', preferredChannel, notes } = req.body
        ...
        const productRows = await appServer.AppDataSource.query('SELECT * FROM product_inventory WHERE "productId" = $1', [productId])
