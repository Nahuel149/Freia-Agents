...
class ProductionTester {
    ...
    async testSampleData() {
        try {
            ...
            // Test customers
            const customersResult = await this.client.query(
                `SELECT id, name, email, company FROM customers LIMIT 3`
            );
    ...
    async testFunctions() {
        try {
            ...
            // Test triggers
            const triggerResult = await this.client.query(
                `SELECT tgname FROM pg_trigger WHERE tgname LIKE '%update_updated_at%'`
            );
