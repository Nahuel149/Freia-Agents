...
class PostgreSQLValidator {
    ...
    async validateTables() {
        ...
        const requiredTables = [
            'chat_flow', 'chat_message', 'credential', 'tool', 'api_key', 'assistant',
            'user', 'organization', 'role', 'login_method', 'organization_user', 
            'workspace', 'workspace_user', 'login_activity'
        ];
    ...
    async validateData() {
        ...
        try {
            ...
            const orgResult = await this.pool.query(
                'SELECT COUNT(*) as count FROM organization WHERE name = $1',
            ...
            const roleResult = await this.pool.query(
                'SELECT COUNT(*) as count FROM role WHERE name = $1',
                ['Admin']
            );
